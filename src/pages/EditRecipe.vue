<template>
<card-layout :pageName='"Editar - " + recipe.name' urlFoto='ovos.jpeg'>
      <Carousel :imgs='recipe.photos' :edit='true' />
        ...
        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 q-gutter-md">
            ...
            <q-input @input="val => { form.files = val }" multiple filled type="file" hint="Imagens da receita"  class="required"/>
            <q-input v-model="form.ingredients" filled type="textarea" label="Ingredientes" :rules="[val => !!val || 'Escreva os ingredientes!']" />
        </div>
        <q-btn type='submit' color="secondary" :loading="loading" class="full-width q-mt-lg" label="Atualizar" />
    </q-form>
</card-layout>
</template>
...
export default {
  ...
  methods: {
    setValues () {
      ...
      this.form.preparation_time = this.recipe.preparation_time
      this.form.preparation_mode = this.recipe.preparation_mode
      this.form.ingredients = this.recipe.ingredients
      this.form.yield = this.recipe.yield
      this.form.category = this.recipe.categories
    },
    async onSubmit () {
      this.loading = true
      ...
      await this.$store.dispatch('recipes/updateRecipe', { recipe_id: this.id, fd: fd })
